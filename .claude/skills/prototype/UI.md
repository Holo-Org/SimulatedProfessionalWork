...
Once a variant has won, capture the answer (which variant and why), then capture the prototype the way the [SKILL](SKILL.md) describes. Fold the winner into the real code and move the rest onto the throwaway revision, not into the main line:

- **Sub-shape A**: fold the winner into the existing page; drop the losing variants and the switcher from the main line.
- **Sub-shape B**: promote the winning variant to a real route; drop the throwaway route and the switcher from the main line.

The full set of variants is the primary source, so it lands on the throwaway revision (bookmarked `prototype/<name>`), not the bin, since variant components and the switcher left on the main line rot fast and confuse the next reader.
